...
const HOLIDAY_THEME_DEFINITIONS = [
  {
    ...
    priority: 90,
    label: "Новый год",
    particles: ["snowflake", "confetti", "gift"],
    density: "high",
    canvas: null,
  },
  {
    ...
    priority: 60,
    label: "Рождество",
    particles: ["star", "snowflake", "candle"],
    density: "medium",
    canvas: null,
  },
  {
  ...
  },
  {
    ...
    afterDays: 1,
    priority: 55,
    label: "День космонавтики",
    particles: ["rocket", "planet"],
    density: "medium",
    canvas: null,
  },
  {
  ...
  },
  {
    ...
    afterDays: 2,
    priority: 100,
    label: "Новогодняя ночь",
    particles: ["snowflake", "confetti"],
    density: "ultra",
    canvas: "fireworks",
  },
];
...
function resolveHolidayDate(definition, year) {
  if (definition.floating === "maslenitsa") {
    return getMaslenitsaDate(year);
  }
...
function getHolidayThemeForDate(date) {
  ...
  HOLIDAY_THEME_DEFINITIONS.forEach((definition) => {
    years.forEach((year) => {
